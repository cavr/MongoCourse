...
public class BlogPostDAO {
    ...
    public DBObject findByPermalink(String permalink) {
        ...
        DBCollection collection = postsCollection;
        
        collection.ensureIndex(new BasicDBObject("permalink", 1).append("unique", true));
        DBObject post = collection.findOne(new BasicDBObject("permalink", permalink));
    ...
    public List<DBObject> findByDateDescending(int limit) {
        ...
        DBCursor cursor = collection.find().sort(new BasicDBObject().append("date", -1)).limit(limit);
        try {
    ...
    public List<DBObject> findByTagDateDescending(final String tag) {
        ...
        DBCursor cursor = collection.find(query).sort(new BasicDBObject().append("date", -1)).limit(10);
