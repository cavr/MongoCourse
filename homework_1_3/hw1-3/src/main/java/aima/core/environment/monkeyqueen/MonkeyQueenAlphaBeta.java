...
public class MonkeyQueenAlphaBeta implements AdversarialSearch<MonkeyQueenState, MonkeyQueenAction> {

	Game <MonkeyQueenState, MonkeyQueenAction, Integer> game;
	private int expandedNodes;
	private int depthLimit;
	
	public MonkeyQueenAlphaBeta(Game<MonkeyQueenState, MonkeyQueenAction, Integer> game, int depthLimit) {
		this.game = game;
	...
	public double maxValue(MonkeyQueenState state, Integer player, double alpha, double beta, int currentLimit) {
		...
		if (currentLimit == 0)
			return state.eval(state.getPlayerToMove());
