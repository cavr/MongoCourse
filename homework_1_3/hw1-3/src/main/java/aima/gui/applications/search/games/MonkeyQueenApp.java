...
public class MonkeyQueenApp {
	...
	ActionListener {
		...
		MonkeyQueenPanel() {
			...
			"IterativeAlphaBeta", "DepthLimitedIterativeAB", "InstantBest", "Alpha-Beta" });
				...
				for(int j = 0; j<currState.getCols(); j++){
					...
					if(piece != null){
						if (piece instanceof MonkeyQueenQueen){
							if (piece.getColor() == 0)
							elements[i][j].setIcon(new ImageIcon("queen0.png"));
							else
								elements[i][j].setIcon(new ImageIcon("queen1.png"));
							elements[i][j].setText((Integer.toString(((MonkeyQueenQueen) piece).getStack())));
						}
						else{
							if (piece.getColor() == 0)
							elements[i][j].setIcon(new ImageIcon("0.png"));
							else
								elements[i][j].setIcon(new ImageIcon(("1.png")));
